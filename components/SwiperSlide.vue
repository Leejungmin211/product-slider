  ...
  if (lastSlideConfirm.value) {
    return {
      transform: `translateX(-${
        currentSlide.value * slideWidth.value + lastSlideWidth
      }px)`,
...
<template>
  <div class="relative w-1050">
    <div class="overflow-hidden flex">
      ...
      <div
        v-if="lastSlideConfirm"
        class="absolute top-0 right-0 w-249 h-320 flex justify-center items-center transition-all duration-200 ease-in"
      >
        <div
          class="flex flex-col justify-center items-center opacity-0 -translate-x-2 animate-moveButtonAnimate"
        >
          <button class="mb-4.5">
      ...
      <button
        class="absolute top-40 left-0 z-50 -translate-x-1/2 -translate-y-1/2"
        v-if="currentSlide !== 0"
        @click="preButton"
      >
      ...
      <button
        v-if="!lastSlideConfirm"
        class="absolute top-40 right-0 z-50 translate-x-1/2 -translate-y-1/2 rotate-180"
        @click="nextButton"
      >
